...
set "params=%*"
cd /d "%~dp0" && ( if exist "%temp%\mat3us.vbs" del "%temp%\mat3us.vbs" ) && fsutil dirty query %systemdrive% 1>nul 2>nul || (  echo Set UAC = CreateObject^("Shell.Application"^) : UAC.ShellExecute "cmd.exe", "/k cd ""%~sdp0"" && %~s0 %params%", "", "runas", 1 >> "%temp%\mat3us.vbs" && "%temp%\mat3us.vbs" && exit /B )


::===========================================================================
fsutil dirty query %systemdrive%  >nul 2>&1 || (
echo ==== ERROR ====
echo Este script requer privilÇgios de administrador.
echo Para fazer isso, clique com o bot∆o direito do mouse nesse script e selecione 'Executar como administrador'
echo.
echo 
pause
exit
...
REM ReadMe Start
 ...
 - Este script n∆o Ç de autoria da HC INFORMATICA
 ...
 # CrÇditos: 
   ...
   Abbodi1406    - Autor do SppExtComObjPatcher-kms \ 2-Activate-Local.cmd que usou
ˇˇˇˇˇˇˇˇˇˇˇˇˇˇˇˇˇˇˇcomo base neste script online.
				   
   rpo           - Ajuda excelente e cont°nua para melhorar este script.
                   
   AR_Alex       - IdÇias, sugest‰es e ajuda interessantes.
   
   WindowsAddict - Teste, gerenciamento e publicaá∆o.  
...
echo       ============================================================================
echo         Nota: Verifique se vocà est† conectado Ö Internet para uma ativaá∆o bem-sucedida.
echo       ============================================================================
echo.
...
echo      ============================================================================
echo       Nota: Esta opá∆o criar† uma tarefa de renovaá∆o autom†tica semanal.
echo             Seu sistema deve se conectar Ö Internet uma vez a cada seis meses,
echo             para ativaá∆o vital°cia.
echo             O arquivo de tarefas ser† criado em C:\Windows\CodeInjection
echo             E vocà pode ver sua Tarefa agendada no Agendador de tarefas.
echo      ============================================================================
echo.
		
schtasks /delete /tn CodeInjection-Renewal /f 1>nul 2>nul
schtasks /delete /tn CodeInjection-Run_Once /f 1>nul 2>nul
@RD /s /q "%windir%\CodeInjection" >nul 2>&1
md "%windir%\CodeInjection"
...
copy /y "%temp%\temp_echo_file" + "%temp%\temp_cmd_file" "C:\Windows\CodeInjection\CodeInjection-Renewal.cmd" >nul
del /q "%temp%\temp_cmd_file" >nul & del /q "%temp%\temp_echo_file" >nul

call :create_file  %0 "%temp%\CodeInjection-Renewal.xml" "REM (Re)activate xml Start" "REM (Re)activate xml End"

call :create_file  %0 "C:\Windows\CodeInjection\Info.txt" "REM Info Start" "REM Info End"

goto :RenewalTaskCreated


:RenewalTaskCreated
schtasks /create /tn "CodeInjection-Renewal" /ru "SYSTEM" /xml "%TEMP%\CodeInjection-Renewal.xml" >nul 2>&1
del /f /q "%TEMP%\CodeInjection-Renewal.xml" >nul 2>&1)
echo.
echo.
echo =========================================
echo  Tarefa de renovaá∆o de ativaá∆o criada.
echo =========================================
echo.
goto :Exit
...
mode con cols=98 lines=30
echo       ================================================================================
echo        Nota: Esta opá∆o excluir† tarefas de renovaá∆o autom†tica + arquivos de tarefas + limpar cache do KMS 
echo              e nome do servidor KMS do sistema.
echo       ================================================================================
echo.
choice /C:GC /N /M "[C] Continuar [G] Voltar : "
...
echo.
echo ==============================
echo  A desinstalaá∆o completa est† conclu°da.
echo ==============================
echo.
pause
goto:MAINMENU
...
mode con cols=98 lines=30
echo     ==================================================================================
echo      Nota: Esta opá∆o criar† a pasta $ OEM $ deste ativador na †rea de trabalho, 
echo            Que vocà pode usar para criar a instalaá∆o prÇ-ativada do Windows. 
echo     ==================================================================================
echo.
choice /C:GC /N /M "[C] Continuar [G] Voltar : "
        if %errorlevel%==1 Goto:MainMenu
...
echo.
echo               ================================================
echo                 Erro - a pasta $ OEM $ n∆o foi criada porque
echo                       A pasta $ OEM $ j† existe na †rea de trabalho.
echo               ================================================
echo. 
pause 
goto:MAINMENU
...
copy /y "%temp%\temp_echo_file" + "%temp%\temp_cmd_file" "C:\Windows\CodeInjection\CodeInjection-Run_Once.cmd" >nul
del /q "%temp%\temp_cmd_file" >nul & del /q "%temp%\temp_echo_file" >nul

call :create_file  %0 "%temp%\CodeInjection-Renewal.xml" "REM (Re)activate xml Start" "REM (Re)activate xml End"

call :create_file  %0 "%temp%\CodeInjection-Run_Once.xml" "REM Run_Once xml Start" "REM Run_Once xml End"
...
set KMS_Port=1688
If defined Renewal_Task call :Re_Activate>>"%windir%\CodeInjection\logfile.txt"&exit
If defined Run_Once call :Re_Activate>>"%windir%\CodeInjection\logfile.txt"&exit
:Re_Activate
set /a loop=1&set/a max_loop=1
if defined Renewal_Task set /a max_loop=3
if defined Run_Once set /a max_loop=5
:repeat
Echo Verificando a conex∆o com a Internet ...
ping www.google.com -n 1 -w 10000 > nul || (
   if %loop%== %max_loop% (
        echo Internet n∆o est† conectada.
   ...
   echo Aguardando 30 segundos&timeout /t 30>nul
   set /a loop=%loop%+1
   goto repeat
)
echo Conex∆o com a internet ok
...
if %server_num% gtr !max_servers! (
	if defined Renewal_Task (echo Nenhum servidor KMS dispon°vel. Saindo...&exit 1651565635 rem Dummy Numbers To Show Error In Task)
	if defined Run_Once (echo Nenhum servidor KMS dispon°vel. Saindo...&exit 1651565635 rem Dummy Numbers To Show Error In Task)
	echo Nenhum servidor KMS dispon°vel. & pause & goto:EOF)
	
set KMS_IP=!server[%server_num%]!
echo. &echo Tentando com o servidor de ativaá∆o &echo.
...
echo.
if %activation_ok%==0 (
echo A ativaá∆o n∆o teve àxito. Tentando outro servidor.
set /a server_num+=1
goto :server
)
if defined Renewal_Task (echo Exiting...&exit)
if defined Run_Once (schtasks /delete /tn CodeInjection-Run_Once /f 1>nul 2>nul &del /f /q %windir%\CodeInjection\CodeInjection-Run_Once.cmd >nul 2>&1 &echo Exiting...&exit)
Echo.
Goto:EOF
...
if %loc_off15% equ 0 if %loc_off16% equ 0 (set "aword=N∆o instalado") else (set "aword=No Supported KMS Client")
wmic path %spp% where (Description like '%%KMSCLIENT%%') get Name 2>nul | findstr /i Office 1>nul && (set office15=1) || (if not defined win7 echo.&echo %aword% Office 2013/2016/2019 Produto detectado...)
...
wmic path %sps% get Version >nul 2>&1 || (echo.&echo Nenhum office %aword% Produto detectado...&exit /b)
wmic path %spp% where (Description like '%%KMSCLIENT%%') get Name >nul 2>&1 || (echo.&echo Nenhum office de cliente KMS suportado %aword% Produto detectado...&exit /b)
...
for /f "tokens=2 delims==" %%x in ('"wmic path %spp% where ID='%app%' get Name /VALUE"') do echo Verificando: %%x
echo O produto est† permanentemente ativado.
exit /b
)
call :insKey %app%
exit /b
...
if "%key%" EQU "" (echo N∆o foi poss°vel encontrar a chave do cliente KMS correspondente&exit /b)
...
for /f "tokens=2 delims==" %%x in ('"wmic path %spp% where ID='%1' get Name /VALUE"') do echo Ativando: %%x
...
if %gpr% equ 43200 if %office% equ 0 if not defined win7 (echo Ativaá∆o do Windows Core / ProfessionalWMC bem-sucedida&echo Remaining Period: 30 days ^(%gpr% minutes^)&exit /b)
if %gpr% equ 64800 (echo Ativaá∆o do Windows Core / ProfessionalWMC bem-sucedida&echo Per°odo restante: 45 dias ^(%gpr% minutos^)&exit /b)
if %gpr% gtr 259200 (echo Ativaá∆o do Windows EnterpriseG / EnterpriseGN bem sucedida&echo Per°odo restante: %gpr2% dias ^(%gpr% minutos^)&exit /b)
if %gpr% equ 259200 (
echo Ativaá∆o do produto bem sucedida
) else (
call cmd /c exit /b %ERRORCODE%
echo Falha na ativaá∆o do produto: 0x%=ExitCode%
set activation_ok=0
)
echo Per°odo restante: %gpr2% dias ^(%gpr% minutos^)
exit /b

:UnsupportedVersion
echo ==== ERROR ====
echo Vers∆o do Windows n∆o suportada detectada.
echo O projeto Ç suportado apenas no Windows 7/8 / 8.1 / 10 e seu equivalente no servidor.
echo.
pause
goto :eof
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Version>1.0</Version>
    <Description>CodeInjection-Renewal - Weekly Activation Renewal Task</Description>
    <URI>\CodeInjection-Renewal</URI>
  ...
  <Actions Context="LocalSystem">
    <Exec>
      <Command>%windir%\CodeInjection\CodeInjection-Renewal.cmd</Command>
    </Exec>
  </Actions>
</Task>
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Author>rpo/WindowsAddict</Author>
    <Version>1.0</Version>
    <Description>CodeInjection-Run_Once - Run and Delete itself on first Internet Contact</Description>
    <URI>\CodeInjection-Run_Once</URI>
    <SecurityDescriptor>D:P(A;;FA;;;SY)(A;;FA;;;BA)(A;;FRFX;;;LS)(A;;FRFW;;;S-1-5-80-123231216-2592883651-3715271367-3753151631-4175906628)(A;;FR;;;S-1-5-4)</SecurityDescriptor>
  </RegistrationInfo>
  <Triggers>
    <LogonTrigger>
  ...
  <Actions Context="LocalSystem">
    <Exec>
      <Command>%windir%\CodeInjection\CodeInjection-Run_Once.cmd</Command>
    </Exec>
  </Actions>
</Task>
...
::
REM Info Start
O uso desse script Ç renovar automaticamente a Ativaá∆o do Windows e do escrit¢rio.
Vocà pode visualizar sua tarefa agendada em "Agendador de tarefas"

Se vocà deseja script completo, atualizaá‰es e mais informaá‰es,
V† para esta p†gina inicial do script https://www.nsaneforums.com/topic/318518-online-kms-activation-script/

Desfrute de uma ativaá∆o do Windows e do Office mais leve e 100% limpa.
REM Info End
::
:SetupCompleteFinal
schtasks /create /tn "CodeInjection-Renewal" /ru "SYSTEM" /xml "%TEMP%\CodeInjection-Renewal.xml" >nul 2>&1
schtasks /create /tn "CodeInjection-Run_Once" /ru "SYSTEM" /xml "%TEMP%\CodeInjection-Run_Once.xml" >nul 2>&1
del /f /q "%TEMP%\CodeInjection-Renewal.xml" >nul 2>&1)
del /f /q "%TEMP%\CodeInjection-Run_Once.xml" >nul 2>&1)
@RD /S /Q "%windir%\setup\"
exit
...
echo.
echo ======================================================
echo  A pasta $ OEM $ foi criada com àxito na †rea de trabalho.
echo ======================================================
pause
goto:MAINMENU
...
:======================================================================================================================================================
:ScriptUpdates
start https://www.hcinformatica.info/
goto:MAINMENU
...
echo.                        ===========================================
echo.                                                                   
echo.                                https://www.hcinformatica.info/   
echo.                                                                    
echo.                        ===========================================
echo.
echo.
